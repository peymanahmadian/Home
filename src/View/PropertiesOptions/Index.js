...
const Index=()=>{
    ...
    const loadOptions=(properties)=>{
        if(properties){
            axios.get(`${Options.get}/${properties}`,{headers:Header(true)}).then((response)=>{
        ...
        <RTL>
            <Grid className={"header_input"}>
                <div>
                    ...
                    {
                        properties &&
                        <Select value={selectedProperties} onChange={(e)=>setSelectedProperties(e.target.value)}>
                            {properties.map(item=><MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}
                        </Select>
                    }
                </div>
            </Grid>
                    ...
                    actions={
                        {
                            onRowAdd: newData =>
                                new Promise((resolve) => {
                                    ...
                                    axios.post(Options.post,body,{headers:Header(true)}).then((response)=>{
                                        loadOptions(selectedProperties);
                                        resolve()
                                    }).catch(()=>{
                            ...
                            onRowUpdate: (newData) =>
                                new Promise((resolve) => {
                                    axios.put(Options.edit,newData,{headers:Header(true)}).then((response)=>{
                                        loadOptions(selectedProperties);
                                        resolve()
                                    }).catch(()=>{
                            ...
                            onRowDelete: oldData =>
                                new Promise((resolve,reject) => {
                                    ...
                                    axios.delete(Options.delete +"/"+ param,{headers:Header(true)}).then((response)=>{
                                        loadOptions(selectedProperties);
                                        resolve();
                                    }).catch((err)=>{
