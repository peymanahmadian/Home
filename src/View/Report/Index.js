...
const Index=()=>{
    ...
    const [formVal,setFormVal]=useState({phoneNumber:"",cityId:"",neighborhoodId:""});
        ...
        for(let col in data){
            ...
            if(col==="url"){
                colName.push({title:word ,field:col,
                    render:(rowdata)=><img height={"95px"} alt={"home"} src={`http://adv.sandboxdevelop.ir/${rowdata.url}.jpeg`} />});
            }
            else if(col==="fromDate"){
                colName.push({title:word ,field:col,
                    render:(rowdata)=>moment(rowdata.fromDate,"YYYY-MM-DDTHH:mm:SS").format("jYYYY-jMM-jDD")});
            }
            else if(col==="toDate"){
                colName.push({title:word ,field:col,
                    render:(rowdata)=>moment(rowdata.toDate,"YYYY-MM-DDTHH:mm:SS").format("jYYYY-jMM-jDD")});
            }
            else if(col==="id" || col==="userId"){
                colName.push({title:word ,field:col,hidden:true});
            }
            else if(col==="isActive"){
                colName.push({title:word ,field:col,render:(rowdata)=>rowdata.isActive?"فعال":"غیرفعال"});
            }
    ...
    useEffect(()=>{
        if(formVal.cityId){
            axios.get(`${CityPart.get}/${formVal.cityId}`,{headers:Header(true)}).then((response)=>{
                        ...
                        type===Report.types[8].value &&
                            <>
                                <Grid className={"col_item"} item xs={12} sm={6} md={3} lg={3}>
                                    ...
                                    {
                                        city.length>0 ?
                                        <Select fullWidth={true} value={formVal.cityId} onChange={(e)=>setFormVal({...formVal,cityId:e.target.value})}>
                                ...
                                <Grid className={"col_item"} item xs={12} sm={6} md={3} lg={3}>
                                    ...
                                    {
                                        parts.length>0 ?
                                            <Select fullWidth={true} value={formVal.neighborhoodId} onChange={(e)=>setFormVal({...formVal,neighborhoodId:e.target.value})}>
                                                {parts.map(item=><MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}
                                            </Select>
                                            :
                        ...
                        <>
                            <Grid className={"col_item"} item xs={12} sm={6} md={4} lg={4}>
                                ...
                                <Button
                                    ...
                                    type={"button"}
                                    onClick={()=>{
                                        setFormVal(({phoneNumber:"",cityId:"",neighborhoodId:""}));
                                        setFromDate(null);
                                        setToDate(null);
                                    }}
                                >
