            ...
            <Table
                ...
                column={[
                    ...
                    {title:'نوع',field:'type',
                        render:rowdata => <span>{rowdata.type===1&&"Text"}{rowdata.type===2&&"Range"}{rowdata.type===3&&"Options"}{rowdata.type===4&&"Bool"}</span>,
                        editComponent: props => (
                            <Select
                                value={props.value}
                    ...
                    {title:'قیمت گذاری شده',field:'isImportantToPrice',
                        render:(rowdata)=>(rowdata.isImportantToPrice===true)?<CheckIcon/>:<CloseIcon/>,
                        editComponent:props => (
                            <Select
                                value={props.value}
                                onChange={e => props.onChange(e.target.value)}
                    ...
                    {title:'فعال شده',field:'isActive',render:(rowdata)=>(rowdata.isActive)?<CheckIcon/>:<CloseIcon/>,
                    editComponent: props => (
                        <Select
                            value={props.value}
                            onChange={e => props.onChange(e.target.value)}
                ...
                actions={
                    {
                        onRowAdd: newData =>
                            new Promise((resolve,reject) => {
                                ...
                                axios.post(Properties.post,body,{headers:Header(true)}).then((response)=>{
                                    loadData();
                                    resolve()
                                }).catch(()=>{
                        ...
                        onRowUpdate: (newData, oldData) =>
                            new Promise((resolve) => {
                                let body={...newData,"IsActive":true};
                                axios.put(Properties.edit,body,{headers:Header(true)}).then((response)=>{
                                    loadData();
                                    resolve()
                                }).catch(()=>{
                        ...
                        onRowDelete: oldData =>
                            new Promise((resolve,reject) => {
                                ...
                                axios.delete(Properties.delete +"/"+ param,{headers:Header(true)}).then((response)=>{
                                    loadData();
                                    resolve();
                                }).catch((err)=>{
