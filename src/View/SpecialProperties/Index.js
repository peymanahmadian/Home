...
const Index = () => {
  ...
  const loadSpecial = (param) => {
    if (param) {
      setOptions([]);
      axios
        .get(`${Special.get}/${param}`, { headers: Header(true) })
        .then((response) => {
        ...
        <Table
          ...
          column={[
            { title: "شماره", field: "id", editable: "never" },
            {
              ...
              editComponent: (prop) => {
                return (
                  <select
                    value={properties.find((pro) => (pro.name = prop.value)).id}
            ...
            {
              title: "نام دسته بندی سطح اول",
              field: "categoryLevelOneName",
              editable: "never",
            },
            {
              title: "فعال سازی",
              ...
              editComponent: (prop) => (
                ...
                >
                  ...
                  <option value={false}>فعال نشده</option>
                </select>
              ),
            },
          ]}
          actions={{
            onRowAdd: (newData) =>
              new Promise((resolve) => {
                let body = {
                  ...
                  PropertyId: parseInt(newData.propertyId),
                  CategoryLevelOneId: selectedCategory,
                };
                axios
                  .post(Special.post, body, { headers: Header(true) })
                  .then((response) => {
                    loadSpecial(selectedCategory);
                    resolve();
                  })
            ...
            onRowUpdate: (newData) =>
              new Promise((resolve) => {
                axios
                  .put(Special.edit, newData, { headers: Header(true) })
                  .then((response) => {
                    loadSpecial(selectedCategory);
                    resolve();
                  })
