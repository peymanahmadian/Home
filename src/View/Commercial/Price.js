...
import {LevelOne,Special, Header,CommercialTime,CommercialPeriodTime} from '../../Model/Urls';
...
const Index=()=>{
    ...
    const loadPriceCommercial=(selectCat,selectCom)=>{
        if(selectCat && selectCom){
            setPricedCommercial([]);
            axios.get(`${CommercialPeriodTime.get}/${selectCat}/${selectCom}`,{headers:Header(true)}).then((response)=>{
                setPricedCommercial([response.data.data]);
            }).catch(()=>{setPricedCommercial([])})
        }
                ...
                <div>
                    <label htmlFor="selectOne">سطح زمانی انتخاب شده:    </label>
                    {
                        ...
                        <Select value={selectedCommercial} onChange={(e)=>setSelectedCommercial(e.target.value)}>
                            {commercial.map(item=><MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}
                        </Select>
                    }
                    ...
                    actions={
                        {
                            onRowAdd: newData =>
                                new Promise((resolve) => {
                                    let body={...newData,"PeriodId":selectedCommercial,"CategoryLevelOneId":selectedCategory}
                                    axios.post(CommercialPeriodTime.post,body,{headers:Header(true)}).then((response)=>{
                                        loadPriceCommercial(selectedCategory,selectedCommercial);
                                        resolve()
                                    }).catch(()=>{
                            ...
                            onRowUpdate: (newData) =>
                                new Promise((resolve) => {
                                    ...
                                    axios.put(CommercialPeriodTime.edit,newData,{headers:Header(true)}).then((response)=>{
                                        loadPriceCommercial(selectedCategory,selectedCommercial);
                                        resolve()
                                    }).catch(()=>{
                            ...
                            onRowDelete: oldData =>
                                new Promise((resolve,reject) => {
                                    ...
                                    axios.delete(CommercialPeriodTime.delete +"/"+ param,{headers:Header(true)}).then((response)=>{
                                        loadPriceCommercial(selectedCategory,selectedCommercial);
                                        resolve();
                                    }).catch((err)=>{
