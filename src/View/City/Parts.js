...
const Parts = () => {
  ...
  const loadCityPart = (city) => {
    if (setSelectedCities) {
      axios
        .get(`${CityPart.get}/${city}`, { headers: Header(true) })
        .then((response) => {
          ...
          actions={{
            onRowAdd: (newData) =>
              new Promise((resolve, reject) => {
                ...
                axios
                  .post(CityPart.post, body, { headers: Header(true) })
                  .then((response) => {
                    loadCityPart(selectedCity);
                    resolve();
                  })
            ...
            onRowUpdate: (newData) =>
              new Promise((resolve) => {
                let body = { ...newData, IsActive: true, CityId: selectedCity };
                axios
                  .put(CityPart.edit, body, { headers: Header(true) })
                  .then((response) => {
                    loadCityPart(selectedCity);
                    resolve();
                  })
